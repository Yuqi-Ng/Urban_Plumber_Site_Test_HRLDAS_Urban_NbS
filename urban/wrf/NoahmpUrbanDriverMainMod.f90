...
contains
  ...
  SUBROUTINE noahmp_urban( NoahmpIO,sf_urban_physics,         NSOIL,     IVGTYP,ITIMESTEP,  & ! IN : Model configuration 
                             ...
                             SWDDIR,         SWDDIF,       LAI_TREE,    LAI_VEG,            & ! added by yuqi
                      ...
                      urban_map_gbd,  urban_map_fbd, urban_map_zgrd,                        & !I multi-layer urban
                       ...
                       sfwin1_urb3d,   sfwin2_urb3d,                                        & !H multi-layer urban
                        ...
                        drain_urb4d,  draingr_urb3d,     sfrv_urb3d, lfrv_urb3d,            & !RMS
                         ...
                         cmcg_urb2d,      tvg_urb2d,       tt_urb2d,xxxvg_urb2d,            & !H urban (add by yuqi)
                     flxhumvg_urb2d,  flxhumt_urb2d,     tvgl_urb3d,  smg_urb3d             & !H urban (add by yuqi)																				 
                 )
...
!----------------------------------------------------------------
    ...
    REAL(kind=kind_noahmp),    DIMENSION( ims:ime,          jms:jme ), INTENT(IN   ) ::  LAI_TREE  ! lai of urban tree [m m-1]    ! added by yuqi
    REAL(kind=kind_noahmp),    DIMENSION( ims:ime,          jms:jme ), INTENT(IN   ) ::  LAI_VEG   ! lai of urban grass[m m-1]    ! added by yuqi
...
! local variables lsm --> urban
     ...
     REAL(kind=kind_noahmp)    :: LAI_TREE_URB     ! urban tree lai    [m/m]  added by yuqi
     REAL(kind=kind_noahmp)    :: LAI_VEG_URB      ! urban grass lai   [m/m]  added by yuqi
...
!===hydrological variable for single layer UCM===
     ...
     ! urban nbs variables for single layer UCM (yuqi) ===
     REAL(kind=kind_noahmp)    :: FLXHUMVG_URB
     REAL(kind=kind_noahmp)    :: FLXHUMT_URB
     REAL(kind=kind_noahmp)    :: TVG_URB
     REAL(kind=kind_noahmp)    :: TT_URB
     REAL(kind=kind_noahmp)    :: XXXVG_URB
     REAL(kind=kind_noahmp)    :: CMCG_URB
     REAL(kind=kind_noahmp), DIMENSION(1:num_roof_layers) :: SMG_URB  
     REAL(kind=kind_noahmp), DIMENSION(1:num_roof_layers) :: TVGL_URB 

     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, jms:jme ),                    INTENT(INOUT) :: FLXHUMVG_URB2D
     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, jms:jme ),                    INTENT(INOUT) :: FLXHUMT_URB2D
     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, jms:jme ),                    INTENT(INOUT) :: TVG_URB2D
     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, jms:jme ),                    INTENT(INOUT) :: TT_URB2D
     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, jms:jme ),                    INTENT(INOUT) :: XXXVG_URB2D
     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, jms:jme ),                    INTENT(INOUT) :: CMCG_URB2D
     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, 1:num_road_layers, jms:jme ), INTENT(INOUT) :: TVGL_URB3D
     REAL(kind=kind_noahmp), OPTIONAL, DIMENSION( ims:ime, 1:num_road_layers, jms:jme ), INTENT(INOUT) :: SMG_URB3D																																													  
...
ILOOP : DO I = its, ite
  ...
  IF( IVGTYP(I,J) == NoahmpIO%ISURBAN_TABLE .or. IVGTYP(I,J) == NoahmpIO%LCZ_1_TABLE .or. &
    ...
    LAI_TREE_URB  = LAI_TREE(I,J)                    ! [m/m]   added by yuqi 
    LAI_VEG_URB   = LAI_VEG(I,J)                     ! [m/m]   added by yuqi
    ...
    TT_URB      = TT_URB2D(I,J)      ! add by yuqi
    ...
    DO K = 1,num_road_layers
	  ...
	  TVGL_URB(K)= TVGL_URB3D(I,K,J) ! add by yuqi
    ...
    CALL urban(LSOLAR_URB,                                                             & ! I
         ...
         GZ1OZ0_URB,                                                                   & ! O
          ...
          DRELG_URB,FLXHUMR_URB,FLXHUMB_URB,FLXHUMG_URB,                               &
            TVG_URB,     TT_URB,  XXXVG_URB,   TVGL_URB,    SMG_URB, CMCG_URB,         & ! H  add by yuqi
       FLXHUMVG_URB, FLXHUMT_URB, LAI_TREE_URB, LAI_VEG_URB)  												 ! addd by yuqi										  
...
! Renew Urban State Variables
    ...
    TVG_URB2D(I,J)      = TVG_URB        ! add by yuqi
    TT_URB2D(I,J)       = TT_URB         
    ...
    DO K = 1,num_road_layers
      ...
      TVGL_URB3D(I,K,J) = TVGL_URB(K)  ! add by yuqi
