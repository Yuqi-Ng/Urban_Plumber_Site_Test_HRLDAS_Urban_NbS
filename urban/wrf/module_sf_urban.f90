...
!===Huang and Wang, 2025 Dec, urban nature-based solutions for single layer UCM (including trees)===
   INTEGER                         :: TREEOPTION                        ! Urban Tree&Vegetation Option ! add by yuqi
   REAL(kind=kind_noahmp)                            :: FVG_TBL         ! Vegetated Fraction for Ground 
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: RTREE_TBL       ! Tree crown radius [normalized]
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: RTREE_M_TBL     ! Tree crown radius in meter
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: RW_M_TBL        ! Road width in meter																		   
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: HTREE_TBL       ! Tree crown center height [normalized]
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: DTREE_TBL       ! Tree crown center distance from wall (half canyon width currently) [normalized]
   !REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: LAI_TREE_TBL   ! Tree leaf area index     modified by yuqi
   !REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: LAI_VEG_TBL    ! Vegetated ground leaf area index
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: Z0VG_TBL        ! Roughness length for momentum of vegetated ground
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: Z0HVG_TBL       ! Roughness length for heat of vegetated ground
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: CAPVG_TBL       ! Heat capacity of vegetated ground
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: EPSVG_TBL       ! Emissivity of vegetated ground
   REAL(kind=kind_noahmp), ALLOCATABLE, DIMENSION(:) :: EPST_TBL        ! Emissivity of tree canopy
...
!===============================================================================
   ...
   SUBROUTINE urban(LSOLAR,                                           & ! L
                    ...
                    DRELR,DRELB,DRELG,FLXHUMR,FLXHUMB,FLXHUMG,        & ! H
					     TVG,TT,XXXVG,TVGL,SMG,CMCG,FLXHUMVG,FLXHUMT,      & ! H (add by huang)
					     LAI_TREE,LAI_VEG)

   IMPLICIT NONE

   REAL(kind=kind_noahmp), PARAMETER    :: CP=0.24          ! heat capacity of dry air  [cgs unit]    ! cal/g/c (yuqi)
   REAL(kind=kind_noahmp), PARAMETER    :: EL=583.          ! latent heat of vaporization [cgs unit]  ! cal/g (yuqi)
...
! -----------------------------------  Newly Define Urban Vegetation (NbS) Related Variables (Yuqi) -------------------------------------
   ...
   REAL(kind=kind_noahmp), INTENT(INOUT):: TVG           ! Vegetated Ground Temperature (At Previous Time Step) [K]
   REAL(kind=kind_noahmp), INTENT(INOUT):: TT            ! Tree Temperature (At Previous Time Step) [K]
   REAL(kind=kind_noahmp), INTENT(INOUT):: XXXVG         ! Monin-Obkhov length for Vegetated Ground   [dimensionless]
   REAL(kind=kind_noahmp), DIMENSION(1:num_road_layers), INTENT(INOUT):: TVGL          ! Vegetated Ground Temperature (multi_layer approach)
   REAL(kind=kind_noahmp), DIMENSION(1:num_road_layers), INTENT(INOUT) :: SMG          ! Soil Moisture at Each Layer on Vegetated Ground [-]
   REAL(kind=kind_noahmp), INTENT(INOUT):: CMCG          ! Canopy Intercepted Water on Vegetated Ground
   REAL(kind=kind_noahmp), INTENT(INOUT):: FLXHUMVG, FLXHUMT  ! Evaporation Over Vegetated Ground and Tree 
   !-------------------------------------------------------------------------------
   ! L: Local variables
   !-------------------------------------------------------------------------------
   REAL(kind=kind_noahmp) :: TVGP, TTP ! Temperature at Pervious Time Step
   REAL(kind=kind_noahmp) :: ALBGE     ! Effective Surface Albedo of Ground 
   REAL(kind=kind_noahmp) :: ALBVG     ! Surface Albedo of Vegetated Ground
   REAL(kind=kind_noahmp) :: XI        ! Tree Shadow Related Variable
   REAL(kind=kind_noahmp) :: ANGLE, TAN1, TAN2               ! Reference Angles Between Tree and Roof 
   REAL(kind=kind_noahmp) :: SDT       ! Direct Short Wave Radiation incident on Tree
   REAL(kind=kind_noahmp) :: SDG       ! Direct Short Wave Radiation incident on Ground
   REAL(kind=kind_noahmp) :: SDB       ! Direct Short Wave Radiation incident on Wall
   REAL(kind=kind_noahmp) :: TAU       ! Transmittance of Tree Crown
   REAL(kind=kind_noahmp) :: SVG, SVG1, SVG2, ST, ST1, ST2   ! Short Wave Radiation Absorbed by Vegetated Ground and Tree
   REAL(kind=kind_noahmp) :: SLEAF     ! Short Wave Radiation Absorbed by Tree Averaged by Leaf Area 
   REAL(kind=kind_noahmp) :: CHI_SHADED, CHI_SHADED_TREE     ! Shadow Length on the Ground Casted by Wall/Tree 
   REAL(kind=kind_noahmp) :: ETA_SHADED, ETA_SHADED_TREE     ! Shadow Length on the Wall Casted by Wall/Tree
   REAL(kind=kind_noahmp) :: VFWS_TREE,VFWW_TREE,VFTS_TREE,VFWT_TREE,VFGS_TREE,VFWG_TREE,VFGW_TREE,VFGT_TREE,VFTW_TREE,VFTG_TREE   ! urban tree view factor related
   REAL(kind=kind_noahmp) :: BHVG, RIBVG        
   REAL(kind=kind_noahmp) :: ALPHAVG    
   REAL(kind=kind_noahmp) :: CHVG      ! Local Bulk Transfer Coef for Heat For Vegetated Ground
   REAL(kind=kind_noahmp) :: CDVG      ! Local Bulk Transfer Coef for Momentum For Vegetated Ground
   REAL(kind=kind_noahmp) :: SRUN1     ! Surface Runoff for Vegetated Ground (Infiltration Excess)
   REAL(kind=kind_noahmp) :: SRUN2     ! Subsurface runoff for 2nd soil layer
   REAL(kind=kind_noahmp) :: SRUN3     ! Runoff Within Soil Layers
   REAL(kind=kind_noahmp), DIMENSION(1:num_road_layers) :: ZSOILG    ! Total Depth of Vegetated Ground (Negative) [m], (Accumulated From Surface)
   REAL(kind=kind_noahmp), DIMENSION(1:num_road_layers) :: SMGP      ! Soil Moisture at Each Layer on Vegetated Ground at Pervious Time Step
   REAL(kind=kind_noahmp), DIMENSION(1:num_road_layers) :: ETG       ! Vegetated Canopy Evapotranspiration
   REAL(kind=kind_noahmp), DIMENSION(1:num_road_layers) :: DZVG      ! Depth of Each Single Layer (Positive)[m]    ! This Could be an Input Variable (yuqi)
   REAL(kind=kind_noahmp) :: QS0VG, DQS0VGDTVG, QS0T, DQS0TDTT       ! Saturation QS and derivative for vegetated ground/tree
   REAL(kind=kind_noahmp) :: EPVG      ! Potential Evaporation For Vegetated Ground [kg/m2/s]
   REAL(kind=kind_noahmp) :: EDIRG     ! Direct Evaporation From Vegetated Ground
   REAL(kind=kind_noahmp) :: ETTG      ! Vegetated Canopy Evapotranspiration
   REAL(kind=kind_noahmp) :: ECG       ! Evaporation from Canopy Interception
   REAL(kind=kind_noahmp) :: ETAG      ! Actual(Total) ET For Vegetated Ground
   REAL(kind=kind_noahmp) :: DRIPG     ! Canopy Drippage Into the Ground
   REAL(kind=kind_noahmp) :: BETVG     ! Reduction Factor for Calculating Latent Heat Flux 
   REAL(kind=kind_noahmp) :: VFAC     ! Vegetation Fraction
   REAL(kind=kind_noahmp) :: EMITW,EMITG,EMITT               ! Emitted Longwave Radiation From One Wall/Ground/Tree
   REAL(kind=kind_noahmp) :: EPSGE     ! Effective Surface Emissivity of Ground
   REAL(kind=kind_noahmp) :: RVG1,RVG2,RVG,RT1,RT2,RT   ! Longwave Radiation Received by Vegetated Ground/Tree 
   REAL(kind=kind_noahmp) :: RLEAF     ! Longwave Radiation Received by Tree averaged by Leaf Area
   ...
   REAL(kind=kind_noahmp) :: HVG       ! Sensible Heat Flux by Vegetated Ground
   REAL(kind=kind_noahmp) :: HT        ! Sensible Heat Flux by Tree
   REAL(kind=kind_noahmp) :: RA_LEAF   ! Leaf Boundary Layer Resistent (Empirical Calculated)
   REAL(kind=kind_noahmp) :: ALPHAT    ! sensible heat transfer coefficient for urban trees
   REAL(kind=kind_noahmp) :: W2        ! Deep Soil Moisture
   REAL(kind=kind_noahmp) :: RS_LEAF   ! Leaf Stomatal Resistance
   REAL(kind=kind_noahmp) :: ELET      ! Latent Heat of Tree Leaf    ! w/m/m (yuqi)
   REAL(kind=kind_noahmp) :: AVAILABLE_WATER_SUM, POTENTIAL_ELET     ! newly added by yuqi
   REAL(kind=kind_noahmp) :: QS0C      ! Saturation specific humidity at canyon air temperature
   CHARACTER(LEN=256)   :: WARN_MSG ! A character string for the warning message
   REAL(kind=kind_noahmp), DIMENSION(1:num_road_layers) :: SROOT, SROOTP     ! Root Water Uptake (m/s)
   ...
   REAL(kind=kind_noahmp) :: DQCDTVG,DQCDTT,ELEVG,KVG,G0VG,DG0BDTVG,DG0BDTT,DG0GDTVG,DG0GDTT,DG0VGDTB,DG0VGDTG,DG0VGDTVG,DG0VGDTT ! derivatives wrt temperature
   REAL(Kind = 8) :: WF,A,B,C,E,FF,GG,H,VGF,I,J,O,P,CHARA,det,det_tol,det_scale      !,RVGT  
   REAL(Kind = 8) :: A_use,FF_use,KK_use,lambda,relax
   REAL(kind=kind_noahmp) :: max_step
   REAL(kind=kind_noahmp) :: SSOILG  ! Soil Heat Flux output from SHFLX W/m2  
   REAL(kind=kind_noahmp) :: TGE, TGEP, QS0GE, ALPHAGE, FLXTHVG, FLXTHT ! effective ground values
   REAL(kind=kind_noahmp) :: CMCGP     ! Canopy Intercepted Water on Vegetated Ground
   !----------- fixed universe parameters for urban vegetation  ---------------
   INTEGER, PARAMETER :: NVG   = 4         ! Total Layer of Vegetated Ground
   REAL(kind=kind_noahmp), PARAMETER    :: ALBT  = 0.2       ! Leaf Surface Albedo
   REAL(kind=kind_noahmp), PARAMETER    :: DLEAF = 0.1       ! Unit = m
   REAL(kind=kind_noahmp), PARAMETER    :: TREF  = 298.0     ! Unit = K
   REAL(kind=kind_noahmp), PARAMETER    :: EREF  = 3167.0    ! Unit = K
   
   !----------- local variables from read_para (Yuqi)  ---------------
   REAL(kind=kind_noahmp)                    :: FVG           ! vegetated ground fraction
   REAL(kind=kind_noahmp)                    :: Z0VG          ! Roughness Length for Momentum of Vegetated Ground
   REAL(kind=kind_noahmp)                    :: Z0HVG         ! Roughness Length for Heat of Vegetated Ground  
   REAL(kind=kind_noahmp)                    :: CAPVG         ! Heat Capacity of Vegetated Ground
   REAL(kind=kind_noahmp)                    :: EPSVG         ! Vegetated Ground Emissivity
   REAL(kind=kind_noahmp)                    :: EPST          ! Tree Emissivity
   REAL(kind=kind_noahmp)                    :: RTREE         ! Radius of Tree Crown Center
   REAL(kind=kind_noahmp)                    :: RTREE_M       ! Radius of Tree Crown Center in meters	
   REAL(kind=kind_noahmp)                    :: RW_M          ! road width in meters																	   
   REAL(kind=kind_noahmp)                    :: DTREE         ! Distance of Tree Crown Center From Wall (RW/2)
   REAL(kind=kind_noahmp)                    :: HTREE         ! Height of Tree Crown Center
   !REAL(kind=kind_noahmp)                    :: LAI_VEG       ! Vegetated Ground Leaf Areal Index                   ! modified by yuqi
   !REAL(kind=kind_noahmp)                    :: LAI_TREE      ! Leaf Area Index of Trees (Could be an input)
...
!-------------------------------------------------------------------------------
   ...
   REAL(kind=kind_noahmp), INTENT(IN)    :: LAI_VEG       ! Vegetated Ground Leaf Areal Index            ! modified by yuqi
   REAL(kind=kind_noahmp), INTENT(IN)    :: LAI_TREE      ! Leaf Area Index of Trees (Could be an input)
!-------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------
   ...
   REAL(kind=kind_noahmp) :: DTB, DTG, DTC, DTVG, DTT  ! added by yuqi
...
!   REAL :: DF1, RGR, RGRR, RCH, RR1, RR2, YY, ZZ1, SSOILR
   REAL(kind=kind_noahmp) :: DF1, RGR, RGRR, RCH, YY, ZZ1, SSOILR, RVGT ! added by yuqi
...
! Yang, 2014/10/08, cal. alh
   ...
   ! print* , 'LAI TREE is :', LAI_TREE  ! testing purpose
   ...
   CALL read_param(UTYPE,ZR,SIGMA_ZED,Z0C,Z0HC,ZDC,SVF,R,RW,HGT,  &
                   ...
                   RTREE,RTREE_M,RW_M,HTREE,DTREE,                &        ! modified by yuqi comment out LAI_TREE,LAI_VEG
                   Z0VG,Z0HVG,CAPVG,EPSVG,EPST,FVG,               &        ! added by yuqi																						  
!for BEP
...
!-------------------------------------------------------------------------------
   ...
   RHO=RHOO*0.001            ! air density at first atmospheric level [g/cm3]  - yuqi
...
! SHADOW has not been activated in previous versions of SLUCM (chenghao) - it's not turned on
   ...
   IF (SSG > 0.0) THEN
     ...
     ELSEIF (TREEOPTION .ne. 1) THEN         ! shadow effects model without urban vegetations
       ...
       SG2=SB1*ALBB/(1.-ALBB)*VFGW*(1.-ALBG)
       SB2=SG1*ALBG/(1.-ALBG)*VFWG*(1.-ALBB) + SB1*ALBB*VFWW         ! newly added last term (yuqi)
     ELSE                                                            ! shadow effects model with urban vegetations (yuqi)
       ! ------------------------------------------------------------------------------------------------------------------  
       ! tree module only activated under shadow case
       ! ------------------------------------------------------------------------------------------------------------------ 
       IF ( HTREE+RTREE .gt. ZR) THEN   ! added by yuqi
           call wrf_error_fatal( "tree crown protrude roof height, adjust tree size" )
       END IF
...
! ----------------------------------------------------------------------
       ...
       RGR = EPSV*(RX-SIG*(TGRP**4.)/60.)    ! change TA to TGRP to here (Yuqi)
   ...
   !-------------------------------------------------------------------------------
    ...
    IF (TS_SCHEME == 1) THEN
      ...
      DO ITERATION=1,100
         ...
         ! ALPHAs in the unit of cal/cm2/s/K 
         ...
         IF (ABS(det) < det_tol) THEN
            ...
            END IF
            call wrf_message( "WARNING: Near-singular matrix in Urban Tree Energy Budget; regularizing solve and relaxing update" )
         END IF
    ...
    ELSE
       ...
       CALL force_restore(CAPB,AKSB,DELT,SB,RB,HB,ELEB,TBLEND,TBP,TB)
       CALL force_restore(CAPG,AKSG,DELT,SG,RG,HG,ELEG,TGLEND,TGP,TG)
       CALL force_restore_tree(DELT,ST,RT,HT,ELET/(697.7*60),LAI_TREE,TTP,TT) ! to be fixed (Yuqi 01/21/2026)
...
!===============================================================================
   SUBROUTINE read_param(UTYPE,                                        & ! in
                         ...
                         RTREE,RTREE_M,RW_M,HTREE,DTREE,               & ! out   ! modified by yuqi   commented LAI_TREE,LAI_VEG
                         Z0VG,Z0HVG,CAPVG,EPSVG,EPST,FVG,                   & ! out   ! added by yuqi																								
!for BEP
...
!end BEP
   ...
   REAL(kind=kind_noahmp), INTENT(OUT)    :: ZR,Z0C,Z0HC,ZDC,SVF,R,RW,HGT,AH,ALH,          &
                           ...
                           DTREE,Z0VG,Z0HVG,CAPVG,EPSVG,EPST,FVG   ! modified by yuqi		commented LAI_TREE,LAI_VEG																					   
...
!m   FRC_URB= FRC_URB_TBL(UTYPE)
   ...
   ! LAI_TREE=LAI_TREE_TBL(UTYPE)  ! modified by yuqi
...
!end BEP
  ...
  if(USE_WUDAPT_LCZ.eq.0)then   !AndreaLCZ
   OPEN (UNIT=11,                &
         ...
         IF (IOSTATUS > 0) THEN
         call wrf_error_fatal( 'Error opening URBPARM.TBL. Make sure URBPARM.TBL (found in run/) is linked to the running directory.' )
         ENDIF
   ...
   OPEN (UNIT=11,                &
         ...
         IF (IOSTATUS > 0) THEN
         call wrf_error_fatal( 'Error opening URBPARM_LCZ.TBL. Make sure URBPARM_LCZ.TBL (found in run/) is linked to the running directory.' )
         ENDIF
 endif
   ...
   READLOOP : do 
      ...
      if (name == "Number of urban categories") then
         ...
         IF (.not. ALLOCATED(ZR_TBL)) then
            ...
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating RTREE_TBL in urban_param_init' )
			   ALLOCATE( RTREE_M_TBL(ICATE), stat=allocate_status )                                                  ! added by yuqi
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating RTREE_M_TBL in urban_param_init' )		
            ALLOCATE( RW_M_TBL(ICATE), stat=allocate_status )                                                  ! added by yuqi
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating RW_M_TBL in urban_param_init' )																											 																			
            ALLOCATE( HTREE_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating HTREE_TBL in urban_param_init' )
            ALLOCATE( DTREE_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating DTREE_TBL in urban_param_init' )
            !ALLOCATE( LAI_TREE_TBL(ICATE), stat=allocate_status )                                                     ! modified by yuqi
            !if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating LAI_TREE_TBL in urban_param_init' )
            !ALLOCATE( LAI_VEG_TBL(ICATE), stat=allocate_status )
            !if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating LAI_VEG_TBL in urban_param_init' )
            ALLOCATE( Z0VG_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating Z0VG_TBL in urban_param_init' )
            ALLOCATE( Z0HVG_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating Z0HVG_TBL in urban_param_init' )
            ALLOCATE( CAPVG_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating CAPVG_TBL in urban_param_init' )
            ALLOCATE( EPSVG_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating EPSVG_TBL in urban_param_init' )
            ALLOCATE( EPST_TBL(ICATE), stat=allocate_status )
            if(allocate_status /= 0) call wrf_error_fatal( 'Error allocating EPST_TBL in urban_param_init' )
...
!end BEP
      ...
      !else if (name == "LAI_TREE") then                           ! modified by yuqi
...
!===========================================================================
   SUBROUTINE urban_var_init(ISURBAN, TSURFACE0_URB,TLAYER0_URB,TDEEP0_URB,IVGTYP,  & ! in
                             ...
                             TVG_URB2D,XXXVG_URB2D,TVGL_URB3D,SMG_URB3D,       & ! inout (add by Yuqi Huang)
                             TT_URB2D,CMCG_URB2D,FLXHUMVG_URB2D,FLXHUMT_URB2D, & ! inout (add by Yuqi Huang)																																									
...
!-----------add by Yuqi Huang ----------------
   REAL(kind=kind_noahmp), DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: TVG_URB2D
   REAL(kind=kind_noahmp), DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: TT_URB2D
   REAL(kind=kind_noahmp), DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: XXXVG_URB2D
   REAL(kind=kind_noahmp), DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: CMCG_URB2D
   REAL(kind=kind_noahmp), DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: FLXHUMVG_URB2D
   REAL(kind=kind_noahmp), DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: FLXHUMT_URB2D
   REAL(kind=kind_noahmp), DIMENSION(ims:ime, 1:num_soil_layers, jms:jme), INTENT(INOUT) :: TVGL_URB3D
   REAL(kind=kind_noahmp), DIMENSION(ims:ime, 1:num_soil_layers, jms:jme), INTENT(INOUT) :: SMG_URB3D
!---------------- end ------------------	
...
!FS   FRC_URB2D(I,J)=0.
      ...
      FLXHUMB_URB2D(I,J) = 0.
      FLXHUMG_URB2D(I,J) = 0.
      FLXHUMVG_URB2D(I,J) = 0. ! add by yuqi
      FLXHUMT_URB2D(I,J) = 0. ! add by yuqi											 									
      CMCR_URB2D(I,J)  = 0.
      CMCG_URB2D(I,J)  = 0. ! add by yuqi										  
      TGR_URB2D(I,J)=TSURFACE0_URB(I,J)+0.
      TVG_URB2D(I,J)=TSURFACE0_URB(I,J)+0.  ! add by yuqi
      TT_URB2D(I,J) =TSURFACE0_URB(I,J)+0.  ! add by yuqi																			  
      ENDIF
...
!         TRL_URB3D(I,4,J)=TLAYER0_URB(I,4,J)+0.
      ...
      IF ( sf_urban_physics == 1 ) THEN
          ...
          SMR_URB3D(I,3,J)=0.2
          SMR_URB3D(I,4,J)=0.
          SMG_URB3D(I,1,J)=0.2  ! add by yuqi
          SMG_URB3D(I,2,J)=0.2
          SMG_URB3D(I,3,J)=0.2
          SMG_URB3D(I,4,J)=0.0									   
      ENDIF
...
! ----------------------------------------------------------------------
      DO ITR = 1,ITRMX
         ...
         ZETAT = ZT * RLMO                               ! dimensionless stability parameter for heat at surface layer, which is ζh=zh/L
         IF (ILECH .eq. 0) THEN
...
! ----------------------------------------------------------------------
      ...
      REAL(kind=kind_noahmp), DIMENSION(1:NSOIL), INTENT(IN)   :: ZSOIL, ET, SROOT
...
! ----------------------------------------------------------------------
      ...
      REAL(kind=kind_noahmp), DIMENSION(1:NSOIL), INTENT(IN)   :: SMCP, ZSOIL, ET, SROOT
...
! ----------------------------------------------------------------------
      ...
      DO K = 1,NSOIL-1
         ...
         ELSE
            WPLUS = 0.
         END IF
         SMC (K) = MAX ( MIN (STOT,SMCMAX),0.066 )    ! can change 0.066 to 0.07 in order to avoid floating invalid error in root_uptake step (Yuqi)
      END DO
...
! ----------------------------------------------------------------------
      ...
      REAL(kind=kind_noahmp), INTENT(OUT)     :: SSOIL               ! output : soil heat flux, w/m-2 (yuqi)
...
! ----------------------------------------------------------------------   
   SUBROUTINE SHADOW_TREE (XI, H, W, HTREE, RTREE, DTREE, CHI_SHADED, ETA_SHADED, &
      ...
      REAL(kind=kind_noahmp), INTENT(IN)    :: XI , H, W, HTREE, RTREE, DTREE
      REAL(kind=kind_noahmp), INTENT(OUT)   :: CHI_SHADED, ETA_SHADED, CHI_SHADED_TREE, ETA_SHADED_TREE
      REAL(kind=kind_noahmp)                :: XI_A, XI_B, X0, Y0, X1, X2, Y1, Y2, CHI_TREE, ETA_TREE
...
! ----------------------------------------------------------------------   
      ...
      REAL(kind=kind_noahmp), INTENT(IN)    :: RSMIN, SD_FACET, LAI, W2, WR, WS, TA, TS, QA, QSAT
      REAL(kind=kind_noahmp), INTENT(OUT)   :: RS
      REAL(kind=kind_noahmp)                :: F, FSR, F_THETA, TSC, FE, FT, LAI_EFF
      REAL(kind=kind_noahmp), PARAMETER     :: SD_LIM     = 30  
      REAL(kind=kind_noahmp), PARAMETER     :: RSMAX      = 5000 
      REAL(kind=kind_noahmp), PARAMETER     :: HS         = 54.53    
...
! ---------------------------------------------------------------------- 
   ...
   SUBROUTINE LELEAF (LE, SW, LW, RA_LEAF, RS_LEAF, RA, TA, QA, QSAT)
      ...
      REAL(kind=kind_noahmp), INTENT(IN)    :: SW, LW, RA_LEAF, RS_LEAF, RA, TA, QA, QSAT
      REAL(kind=kind_noahmp), INTENT(OUT)   :: LE
      REAL(kind=kind_noahmp)                :: RN, TC, S, ESAT, RH, EA, D
      REAL(kind=kind_noahmp), PARAMETER     :: GAMMA     = 66.1
      REAL(kind=kind_noahmp), PARAMETER     :: CP        = 1005
...
! ---------------------------------------------------------------------- 
   ...
   SUBROUTINE force_restore_tree(DELT,S,R,H,LE,LAI,TSP,TS)
      ...
      REAL(kind=kind_noahmp), INTENT(IN)  :: DELT,S,R,H,LE,LAI,TSP
      REAL(kind=kind_noahmp), INTENT(OUT) :: TS
...
! ---------------------------------------------------------------------- 
   ...
   SUBROUTINE ROOT_UPTAKE(SROOT, NLAYER, SMC, SMCMAX, SMCDRY, RTREE, LAI_TREE, DZG, ZSOIL, ELE, FVG, RW)
      ...
      REAL(kind=kind_noahmp), DIMENSION(1:NLAYER), INTENT(IN)  :: SMC, DZG, ZSOIL
      REAL(kind=kind_noahmp), INTENT(IN)    :: SMCMAX, SMCDRY, RTREE, LAI_TREE, FVG, RW, ELE
      REAL(kind=kind_noahmp), DIMENSION(1:NLAYER), INTENT(OUT) :: SROOT  !(UNIT: M/S)
      REAL(kind=kind_noahmp), DIMENSION(1:NLAYER)  :: NORMQ, ALPHAI, ROOT, PART2
      REAL(kind=kind_noahmp)                :: ALPHABAR , PART1
      REAL(kind=kind_noahmp), PARAMETER     :: QC1    = 0.6
      REAL(kind=kind_noahmp), PARAMETER     :: QC2    = 1.0
      REAL(kind=kind_noahmp), PARAMETER     :: Lv     = 2.26E6
      REAL(kind=kind_noahmp), PARAMETER     :: rWD    = 1.E3  ! density of water [kg/m3]
...
! ----------------------------------------------------------------------
   ...
   ! Input arguments
   REAL(kind=kind_noahmp), INTENT(IN) :: W, H, ht, rt, trans

   ! Output arguments
   REAL(kind=kind_noahmp), INTENT(OUT) :: FGS, FGW, FGT, FWS, FWG, FWW, FWT, FTS, FTG, FTW

   ! Local variables
   REAL(kind=kind_noahmp) :: dx, dy, tau
   REAL(kind=kind_noahmp) :: FWW_emp, FWG_emp, FWS_emp, FSG_emp, FSW_emp, FGS_emp, FGW_emp
   REAL(kind=kind_noahmp) :: FSW, FSG, FST
   REAL(kind=kind_noahmp) :: FWW_cf1, FWS_cf1, FWG_cf1, FWW_cf2, FWS_cf2, FWG_cf2
   REAL(kind=kind_noahmp) :: FGS_cf1, FGW_cf1, FGS_cf2, FGW_cf2
   REAL(kind=kind_noahmp) :: FSW_cf1, FSG_cf1, FSW_cf2, FSG_cf2
   REAL(kind=kind_noahmp) :: y, d1, theta_blocked_ww, theta_range_ww, theta_blocked_ws, theta_range_ws, theta_blocked_wg, theta_range_wg
   REAL(kind=kind_noahmp) :: x, d2, theta_blocked_gs, theta_range_gs, theta_blocked_gw, theta_range_gw
   REAL(kind=kind_noahmp) :: theta_blocked_sw, theta_range_sw, theta_blocked_sg, theta_range_sg
   REAL(kind=kind_noahmp) :: A_S, A_G, A_W, A_T
   REAL(kind=kind_noahmp) :: sum_T, resid_T, A_sum_T
   REAL(kind=kind_noahmp) :: sum_W, resid_W, A_sum_W
   REAL(kind=kind_noahmp) :: sum_G, resid_G, A_sum_G
   REAL(kind=kind_noahmp) :: sum_S, resid_S, A_sum_S
   REAL(kind=kind_noahmp), PARAMETER :: PI = 3.141592653589793
   INTEGER :: i
   ...
   CONTAINS
      ELEMENTAL PURE FUNCTION cdf2D_VF(theta) RESULT(angleCDF)
         REAL(kind=kind_noahmp)               :: angleCDF
         REAL(kind=kind_noahmp), INTENT(IN)   :: theta
