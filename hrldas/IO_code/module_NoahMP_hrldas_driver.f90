    ...
    if (NoahmpIO%restart_filename_requested /= " ") then
       ...
       if ( NoahmpIO%SF_URBAN_PHYSICS > 0 ) then
          ...
          if ( NoahmpIO%SF_URBAN_PHYSICS == 1 ) then  ! single layer urban model
             ...
             ! new varibales for urban vegetation (yuqi)
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull,    "CMCG_URB2D"  ,    NoahmpIO%CMCG_URB2D )
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull,     "TVG_URB2D"  ,    NoahmpIO%TVG_URB2D  )
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull,     "TT_URB2D"   ,     NoahmpIO%TT_URB2D  )
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull,   "XXXVG_URB2D"  ,  NoahmpIO%XXXVG_URB2D  )
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull,"FLXHUMVG_URB2D"  ,NoahmpIO%FLXHUMVG_URB2D )
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull, "FLXHUMT_URB2D"  ,NoahmpIO%FLXHUMT_URB2D  )
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull,    "TVGL_URB3D"  ,   NoahmpIO%TVGL_URB3D  )
             call get_from_restart(xstart, xend, xstart, ixfull, jxfull,     "SMG_URB3D"  ,    NoahmpIO%SMG_URB3D  )														
          endif
       ...
       if ( NoahmpIO%SF_URBAN_PHYSICS > 0 ) then  !urban
          ...
          call urban_var_init(NoahmpIO%ISURBAN, NoahmpIO%TSK, NoahmpIO%TSLB, NoahmpIO%TMN,                                  & !urban
                              ...
                              NoahmpIO%FLXHUMR_URB2D, NoahmpIO%FLXHUMB_URB2D, NoahmpIO%FLXHUMG_URB2D,                       & !urban
	                           NoahmpIO%TVG_URB2D, NoahmpIO%XXXVG_URB2D, NoahmpIO%TVGL_URB3D, NoahmpIO%SMG_URB3D,            & !urban(add by yuqi)
                              NoahmpIO%TT_URB2D, NoahmpIO%CMCG_URB2D, NoahmpIO%FLXHUMVG_URB2D, NoahmpIO%FLXHUMT_URB2D,      & !urban(add by yuqi)																																			   																																																					 
...
! separate file.  So we can open a new one right now.
     ...
     call READFORC_HRLDAS(NoahmpIO%INFLNM_TEMPLATE, NoahmpIO%FORCING_TIMESTEP,                               &
                          ...
                          NoahmpIO%forcing_name_LAIT,NoahmpIO%forcing_name_LAIV,                             & ! added by yuqi
                          NoahmpIO%forcing_name_SN,                                                          &
                          ...
                          NoahmpIO%LAI, NoahmpIO%LAI_TREE,NoahmpIO%LAI_VEG,                                         & ! added by yuqi
                          NoahmpIO%update_lai, NoahmpIO%reset_spinup_date, NoahmpIO%startdate, &
...
!------------------------------------------------------------------------
         ...
         IF(NoahmpIO%SF_URBAN_PHYSICS > 0 ) THEN  !urban
            ...
            call noahmp_urban(NoahmpIO, NoahmpIO%sf_urban_physics, NoahmpIO%NSOIL, NoahmpIO%IVGTYP,             & ! IN : Model configuration 
                            ...
                            NoahmpIO%SWDDIR, NoahmpIO%SWDDIF, NoahmpIO%LAI_TREE, NoahmpIO%LAI_VEG,              & ! added by yuqi
                            ...
                            NoahmpIO%dl_u_bep, NoahmpIO%sf_bep, NoahmpIO%vl_bep,                                & !O multi-layer urban
						          NoahmpIO%cmcg_urb2d, NoahmpIO%tvg_urb2d, NoahmpIO%tt_urb2d, NoahmpIO%xxxvg_urb2d,   & !H urban (add by yuqi)
                            NoahmpIO%flxhumvg_urb2d, NoahmpIO%flxhumt_urb2d, NoahmpIO%tvgl_urb3d,               & !H urban (add by yuqi)
                            NoahmpIO%smg_urb3d                                                                  & !H urban (add by yuqi)
							)																											 
        ...
        if (mod(ITIME*NoahmpIO%noah_timestep, NoahmpIO%output_timestep) == 0 &
           ...
           DEFINE_MODE_LOOP : do imode = 1, 2
            ...
            if ( NoahmpIO%IOPT_RUNSUB == 5 ) then
              ...
              call add_to_output(NoahmpIO%RECHXY     , "RECH"     , "recharge to or from the water table when shallow"             , "mm"     )
...
subroutine lsm_restart()
  ...
  call prepare_restart_file (trim(NoahmpIO%outdir), version, NoahmpIO%igrid, NoahmpIO%llanduse, &
              ...
              NoahmpIO%num_urban_hi,  NoahmpIO%urban_map_zrd,  &
              NoahmpIO%urban_map_zwd, NoahmpIO%urban_map_gd,   &
 ...
 ! irrigation scheme
  ...
  if ( NoahmpIO%SF_URBAN_PHYSICS > 0 ) then
      ...
      if ( NoahmpIO%SF_URBAN_PHYSICS == 1 ) then  ! single layer urban model  
         ...
         call add_to_restart(     NoahmpIO%LF_URB2D  ,     "LF_URB2D", layers="SOIL")
		 ...
		 ! new varibales for urban vegetation (yuqi)
         call add_to_restart(    NoahmpIO%CMCG_URB2D ,    "CMCG_URB2D" )
         call add_to_restart(  NoahmpIO%TVG_URB2D    ,     "TVG_URB2D" )
         call add_to_restart(  NoahmpIO%TT_URB2D     ,      "TT_URB2D" )
         call add_to_restart(  NoahmpIO%XXXVG_URB2D  ,   "XXXVG_URB2D" )
         call add_to_restart( NoahmpIO%FLXHUMVG_URB2D,"FLXHUMVG_URB2D" )
         call add_to_restart(NoahmpIO%FLXHUMT_URB2D  , "FLXHUMT_URB2D" )
         call add_to_restart(   NoahmpIO%TVGL_URB3D  ,    "TVGL_URB3D", layers="SOIL" )
         call add_to_restart(    NoahmpIO%SMG_URB3D  ,     "SMG_URB3D", layers="SOIL" )
      endif

      if ( (NoahmpIO%SF_URBAN_PHYSICS == 2) .or. (NoahmpIO%SF_URBAN_PHYSICS == 3) ) then ! BEP or BEM urban models  
         call add_to_restart(     NoahmpIO%HI_URB2D  ,      "HI_URB2D", layers="UBHI" )
         call add_to_restart(    NoahmpIO%TRB_URB4D  ,     "TRB_URB4D", layers="UZRD" )
         call add_to_restart(    NoahmpIO%TW1_URB4D  ,     "TW1_URB4D", layers="UZWD" )
         call add_to_restart(    NoahmpIO%TW2_URB4D  ,     "TW2_URB4D", layers="UZWD" )
         call add_to_restart(    NoahmpIO%TGB_URB4D  ,     "TGB_URB4D", layers="UBGD" )
         call add_to_restart(   NoahmpIO%SFW1_URB3D  ,    "SFW1_URB3D", layers="UBZD" )
         call add_to_restart(   NoahmpIO%SFW2_URB3D  ,    "SFW2_URB3D", layers="UBZD" )
         call add_to_restart(    NoahmpIO%SFR_URB3D  ,     "SFR_URB3D", layers="UZDF" )
         call add_to_restart(    NoahmpIO%SFG_URB3D  ,     "SFG_URB3D", layers="UNDM" )
      endif

      if ( NoahmpIO%SF_URBAN_PHYSICS == 3 ) then  ! BEM urban model  
         call add_to_restart(   NoahmpIO%TLEV_URB3D  ,    "TLEV_URB3D", layers="UBBD" )
         call add_to_restart(   NoahmpIO%QLEV_URB3D  ,    "QLEV_URB3D", layers="UBBD" )
         call add_to_restart( NoahmpIO%TW1LEV_URB3D  ,  "TW1LEV_URB3D", layers="UBWD" )
         call add_to_restart( NoahmpIO%TW2LEV_URB3D  ,  "TW2LEV_URB3D", layers="UBWD" )
         call add_to_restart(  NoahmpIO%TGLEV_URB3D  ,   "TGLEV_URB3D", layers="UGBD" )
         call add_to_restart(  NoahmpIO%TFLEV_URB3D  ,   "TFLEV_URB3D", layers="UFBD" )
         ...
         call add_to_restart( NoahmpIO%SFWIN1_URB3D  ,  "SFWIN1_URB3D", layers="UBWD" )
         call add_to_restart( NoahmpIO%SFWIN2_URB3D  ,  "SFWIN2_URB3D", layers="UBWD" )
         call add_to_restart(  NoahmpIO%EP_PV_URB3D  ,   "EP_PV_URB3D" )
         call add_to_restart(   NoahmpIO%T_PV_URB3D  ,    "T_PV_URB3D", layers="UZDF" )
         call add_to_restart(    NoahmpIO%TRV_URB4D  ,    "TRV_URB4D" , layers="ZGRD" )
         call add_to_restart(    NoahmpIO%QR_URB4D   ,     "QR_URB4D" , layers="ZGRD" )
         call add_to_restart(   NoahmpIO%QGR_URB3D   ,    "QGR_URB3D"  )
         call add_to_restart(   NoahmpIO%TGR_URB3D   ,    "TGR_URB3D"  )
         call add_to_restart(   NoahmpIO%DRAIN_URB4D ,   "DRAIN_URB4D", layers="UZDF" )
         call add_to_restart( NoahmpIO%DRAINGR_URB3D , "DRAINGR_URB3D" )
         call add_to_restart(    NoahmpIO%SFRV_URB3D ,    "SFRV_URB3D", layers="UZDF" )
         call add_to_restart(    NoahmpIO%LFRV_URB3D ,    "LFRV_URB3D", layers="UZDF" )
         call add_to_restart(     NoahmpIO%DGR_URB3D ,     "DGR_URB3D", layers="UZDF" )
         call add_to_restart(      NoahmpIO%DG_URB3D ,      "DG_URB3D", layers="UNDM" )
         call add_to_restart(     NoahmpIO%LFR_URB3D ,     "LFR_URB3D", layers="UZDF" )
         call add_to_restart(     NoahmpIO%LFG_URB3D ,     "LFG_URB3D", layers="UNDM" )
      endif
  endif
