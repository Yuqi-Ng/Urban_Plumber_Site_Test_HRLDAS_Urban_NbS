...
! -------------------------------------------------
    ...
    if ( .not. allocated (NoahmpIO%LAI_TREE)  ) allocate ( NoahmpIO%LAI_TREE   (XSTART:XEND,        YSTART:YEND) ) ! lai for urban tree [m m-1]   ! added by yuqi
    if ( .not. allocated (NoahmpIO%LAI_VEG)   ) allocate ( NoahmpIO%LAI_VEG    (XSTART:XEND,        YSTART:YEND) ) ! lai for urban grass [m m-1]
    ...
    if ( .not. allocated (NoahmpIO%UT2)     ) allocate ( NoahmpIO%UT2        (XSTART:XEND,        YSTART:YEND) ) ! urban 2m air temperature [oC] ! added by yuqi huang
    if ( .not. allocated (NoahmpIO%UQ2)     ) allocate ( NoahmpIO%UQ2        (XSTART:XEND,        YSTART:YEND) ) ! urban specific humidity  [-]
    if ( .not. allocated (NoahmpIO%USOILM)  ) allocate ( NoahmpIO%USOILM     (XSTART:XEND,        YSTART:YEND) ) ! urban soil moisture (1st layer) [oC] 
    if ( .not. allocated (NoahmpIO%USOILT)  ) allocate ( NoahmpIO%USOILT     (XSTART:XEND,        YSTART:YEND) ) ! urban soil temperature (1st layer)  [-]
    ...
    if ( NoahmpIO%SF_URBAN_PHYSICS > 0 ) then
       ...
       if ( NoahmpIO%SF_URBAN_PHYSICS == 1 ) then  ! single layer urban model  
          ...
          if ( .not. allocated (NoahmpIO%flxhumvg_urb2d)) allocate ( NoahmpIO%flxhumvg_urb2d (XSTART:XEND,        YSTART:YEND) )
          if ( .not. allocated (NoahmpIO%flxhumt_urb2d) ) allocate ( NoahmpIO%flxhumt_urb2d  (XSTART:XEND,        YSTART:YEND) )
          if ( .not. allocated (NoahmpIO%tvgl_urb3d)    ) allocate ( NoahmpIO%tvgl_urb3d     (XSTART:XEND,1:NSOIL,YSTART:YEND) )
          if ( .not. allocated (NoahmpIO%smg_urb3d)     ) allocate ( NoahmpIO%smg_urb3d      (XSTART:XEND,1:NSOIL,YSTART:YEND) )	       
	   endif ! SLUCM
    ...
    NoahmpIO%LAI_TREE        = undefined_real   ! added by yuqi
    NoahmpIO%LAI_VEG         = undefined_real
    ...
    NoahmpIO%UT2             = undefined_real
    NoahmpIO%UQ2             = undefined_real
    NoahmpIO%USOILM          = undefined_real
    NoahmpIO%USOILT          = undefined_real
    ...
    if ( NoahmpIO%SF_URBAN_PHYSICS > 0 ) then
       ...
       if ( NoahmpIO%SF_URBAN_PHYSICS == 1 ) then  ! single layer urban model
		  ...
		  ! new urban variables for urban nbs (yuqi)
          NoahmpIO%cmcg_urb2d    = undefined_real_neg
          NoahmpIO%tvg_urb2d     = undefined_real_neg
          NoahmpIO%tt_urb2d      = undefined_real_neg
          NoahmpIO%xxxvg_urb2d   = undefined_real_neg
          NoahmpIO%flxhumvg_urb2d= undefined_real_neg
          NoahmpIO%flxhumt_urb2d = undefined_real_neg
          NoahmpIO%tvgl_urb3d    = undefined_real_neg
          NoahmpIO%smg_urb3d     = undefined_real_neg
       endif ! SLUCM
       if ( NoahmpIO%SF_URBAN_PHYSICS == 2 .or. NoahmpIO%SF_URBAN_PHYSICS == 3 ) then  ! BEP or BEM urban models
